/**
 * Enhanced ConsolidatedChatHeader with improved drag & drop functionality
 * Includes better visual feedback and drag state management
 */
...
// Types (same as ConsolidatedChatHeader)
...
interface EnhancedConsolidatedChatHeaderProps {
  // Chat context
...
const EnhancedConsolidatedChatHeader: React.FC<EnhancedConsolidatedChatHeaderProps> = ({
  multiChatState,
  selectedChatbot,
  currentChatName,
  humanParticipants,
  ...
  // Enhanced Draggable Agent Chip Component
  const EnhancedDraggableAgentChip: React.FC<{
    participant: any;
    type: 'ai' | 'human';
  }> = ({ participant, type }) => {
    const isAI = type === 'ai';
    ...
    // Make AI agents draggable with enhanced feedback
    const { dragRef, isDragging, dragHandlers } = useAgentDragSource(
      participant.id,
      {
        ...
        agentId: participant.id, // Ensure agentId is available
      },
      !isAI // isHuman
    );
    ...
    useEffect(() => {
      setDragState(prev => ({ ...prev, [participant.id]: isDragging }));
    }, [isDragging, participant.id]);
      ...
      <Tooltip
        ...
        title={
          <Box>
            ...
            {isDragging && (
              <Box sx={{ fontSize: '11px', color: '#22c55e', mt: 0.5 }}>
                🎯 Drop on a message to select behavior
              </Box>
            )}
          </Box>
        }
        arrow
        open={isDragging ? true : undefined} // Keep tooltip open while dragging
      >
        <Box 
          ...
          sx={{
            ...
            px: 1,
            py: 0.5,
            bgcolor: isDragging ? '#22c55e20' : '#334155',
            borderRadius: 1,
            borderLeft: `3px solid ${isDragging ? '#22c55e' : borderColor}`,
            cursor: isAI ? (isDragging ? 'grabbing' : 'grab') : 'pointer',
            opacity: isDragging ? 0.8 : 1,
            transform: isDragging ? 'rotate(2deg) scale(1.05)' : 'none',
            '&:hover': { 
              bgcolor: isDragging ? '#22c55e30' : '#475569',
              transform: isDragging ? 'rotate(2deg) scale(1.05)' : 'scale(1.05)',
            },
            height: 28,
            ...
            boxShadow: isDragging ? `0 0 20px ${borderColor}40` : 'none',
          }}
        >
          {/* Enhanced Drag Indicator for AI agents */}
          {isAI && (
            <DragIndicator sx={{ 
              fontSize: 10, 
              color: isDragging ? '#22c55e' : '#64748b', 
              position: 'absolute',
              top: 2,
              right: 2,
          ...
          <Avatar sx={{
            width: 16,
            height: 16,
            backgroundColor: isDragging ? '#22c55e' : borderColor,
            fontSize: '8px',
            fontWeight: 'bold',
            transition: 'all 0.2s ease',
          }}>
  ...
  // Render participant chip (now uses EnhancedDraggableAgentChip)
  const renderParticipantChip = (participant: any, type: 'ai' | 'human') => {
    return <EnhancedDraggableAgentChip participant={participant} type={type} />;
  };

  // Render overflow indicator (same as original)
  ...
  // Get agent name (same as original)
  const getAgentName = () => {
  ...
  // Get chat name (same as original)
  const getChatName = () => {
    if (isInSharedMode && activeSharedConversation) {
  ...
  // Get participants based on mode (same as original)
  ...
  // Split visible and hidden participants (same as original)
  ...
  return (
    <Box sx={{ 
      ...
      bgcolor: '#1e293b',
      // Add subtle animation when any agent is being dragged
      transition: 'all 0.3s ease',
      boxShadow: Object.values(dragState).some(Boolean) ? '0 4px 20px rgba(34, 197, 94, 0.2)' : 'none',
    }}>
      {/* Main Header Row */}
      <Box sx={{ 
        display: 'flex', 
      ...
      {Object.values(dragState).some(Boolean) && (
        <Box sx={{
          px: 3,
          py: 1,
          ...
          <Typography sx={{ color: '#22c55e', fontSize: '12px', fontWeight: 500 }}>
            Drag in progress - Drop on a message to interact
          </Typography>
        </Box>
      )}
    </Box>
...
export default EnhancedConsolidatedChatHeader;
