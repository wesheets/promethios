...
interface PerplexityWrapperStubProps {
  onComplete?: (agentData: any) => void;
  onCancel?: () => void;
}

const PerplexityWrapperStub: React.FC<PerplexityWrapperStubProps> = ({ onComplete, onCancel }) => {
  ...
  const handleComplete = async () => {
    ...
    const agentData = {
      id: `perplexity-wrapped-${Date.now()}`,
      name: 'Perplexity (Governed)',
      type: 'perplexity-wrapped',
      provider: 'Perplexity',
      governanceLevel: selectedGovernance,
      description: 'Your existing Perplexity account with Promethios governance',
      isWrapped: true,
      status: 'active',
  ...
  const getStepContent = (step: number) => {
    switch (step) {
      case 0:
        return (
          <Box sx={{ textAlign: 'center', py: 4 }}>
            <Extension sx={{ fontSize: 80, color: '#06b6d4', mb: 3 }} />
            <Typography variant="h4" sx={{ color: 'white', mb: 2 }}>
              Install Promethios Extension
            </Typography>
            <Typography variant="body1" sx={{ color: '#a0aec0', mb: 4, maxWidth: 600, mx: 'auto' }}>
              Add governance to your existing Perplexity account with one simple browser extension. 
              Continue using Perplexity exactly as you do now, but with enterprise-grade protection.
            </Typography>
            ...
            <Card sx={{ backgroundColor: '#2d3748', border: '1px solid #4a5568', mb: 4, textAlign: 'left', maxWidth: 500, mx: 'auto' }}>
              <CardContent>
                <Typography variant="h6" sx={{ color: 'white', mb: 2, display: 'flex', alignItems: 'center' }}>
                  <Search sx={{ mr: 2, color: '#06b6d4' }} />
                  What You'll Get
                </Typography>
                <Box sx={{ pl: 3 }}>
                  <Typography variant="body2" sx={{ color: '#a0aec0', mb: 1, display: 'flex', alignItems: 'center' }}>
                    <CheckCircle sx={{ fontSize: 16, mr: 1, color: '#06b6d4' }} />
                    Same Perplexity interface you love
                  </Typography>
                  <Typography variant="body2" sx={{ color: '#a0aec0', mb: 1, display: 'flex', alignItems: 'center' }}>
                    <CheckCircle sx={{ fontSize: 16, mr: 1, color: '#06b6d4' }} />
        ...
        return (
          <Box sx={{ py: 4 }}>
            <Box sx={{ textAlign: 'center', mb: 4 }}>
              <Security sx={{ fontSize: 64, color: '#06b6d4', mb: 2 }} />
              <Typography variant="h4" sx={{ color: 'white', mb: 2 }}>
            ...
            <Grid container spacing={3} sx={{ maxWidth: 800, mx: 'auto' }}>
              {governanceLevels.map((level) => (
                <Grid item xs={12} md={6} key={level.value}>
                  <Card
                    sx={{
                      backgroundColor: selectedGovernance === level.value ? 'rgba(6, 182, 212, 0.1)' : '#2d3748',
                      border: `2px solid ${selectedGovernance === level.value ? '#06b6d4' : '#4a5568'}`,
                      cursor: 'pointer',
      ...
      case 2:
        return (
          <Box sx={{ textAlign: 'center', py: 4 }}>
            <CheckCircle sx={{ fontSize: 80, color: '#06b6d4', mb: 3 }} />
            <Typography variant="h4" sx={{ color: 'white', mb: 2 }}>
              You're All Set!
            </Typography>
            <Typography variant="body1" sx={{ color: '#a0aec0', mb: 4, maxWidth: 600, mx: 'auto' }}>
              Perplexity is now protected with Promethios governance. Visit perplexity.ai and start using it normally - 
              ...
              <CardContent>
                ...
                <Grid container spacing={2}>
                  <Grid item xs={6}>
                    ...
                    <Typography variant="body1" sx={{ color: 'white' }}>Perplexity</Typography>
                  </Grid>
                  <Grid item xs={6}>
                  ...
                  <Grid item xs={6}>
                    ...
                    <Typography variant="body1" sx={{ color: '#06b6d4' }}>Active</Typography>
                  </Grid>
                  <Grid item xs={6}>
            ...
            <Box sx={{ display: 'flex', gap: 2, justifyContent: 'center' }}>
              <Button
                ...
                sx={{
                  borderColor: '#06b6d4',
                  color: '#06b6d4',
                  '&:hover': { borderColor: '#0891b2', backgroundColor: 'rgba(6, 182, 212, 0.1)' }
                }}
              >
                Open Perplexity
              </Button>
      ...
      <Box sx={{ maxWidth: 1000, mx: 'auto' }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 4 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <Search sx={{ fontSize: 32, color: '#06b6d4', mr: 2 }} />
            <Box>
              <Typography variant="h4" sx={{ color: 'white' }}>
                Wrap Perplexity
              </Typography>
              <Typography variant="body1" sx={{ color: '#a0aec0' }}>
                Add governance to your existing Perplexity account
              </Typography>
            </Box>
          </Box>
        </Box>
            ...
            <Step key={label}>
              <StepLabel
                sx={{
                  ...
                  '& .MuiStepLabel-label.Mui-completed': { color: '#06b6d4' }
                }}
              >
                {label}
              </StepLabel>
...
export default PerplexityWrapperStub;
