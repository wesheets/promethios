...
try:
    from src.api.deployment.routes import router as deployment_router
except ImportError:
...
app = FastAPI(
    ...
    version="2.1.0",
    description="HTTP API for executing the Promethios GovernanceCore loop and managing governed wrapped agents."
)
...
try:
    ...
    print(f"Warning: Could not initialize runtime executor: {e}")
...
async def execute_loop(request: Request):
    ...
    # If runtime executor is not available, return a mock response
        ...
        return JSONResponse(
            ...
            content={
                ...
                "governance_core_output": {
                    ...
                    "received_override": request_body.get("operator_override_signal")
                },
                "emotion_telemetry": {"status": "generated", "emotions": []},
                "justification_log": {"status": "generated", "justifications": []},
                "error_details": None
            }
        )
    ...
    if loop_execute_request_schema:
        try:
    ...
    if operator_override_signal is not None and operator_override_schema: # Ensure it's not just present but also not null if schema expects object
...
@app.get("/health", summary="System Health", tags=["System"])
async def health_check():
    return {
        ...
        "services": {
            "api": "healthy",
            "observers": "healthy" if observers_router else "unavailable",
            "agents": "healthy" if agents_router else "unavailable",
            "multi_agent": "healthy" if multi_agent_router else "unavailable",
            "audit": "healthy" if audit_router else "unavailable",
            "policy": "healthy" if policy_router else "unavailable",
            "deployment": "healthy" if deployment_router else "unavailable",
            "monitoring": "healthy" if monitoring_router else "unavailable"
        }
    }
