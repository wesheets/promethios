...
def call_governance_identity(method: str, agent_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        wrapper_script = f"""
const {{ GovernanceIdentity }} = require('{script_path}');
const identity = new GovernanceIdentity({{logger: console}});

const agentData = {json.dumps(agent_data)};
const result = identity.{method}(agentData);
console.log(JSON.stringify(result));
"""
        ...
        # Find the JSON result (last line that starts with {)
        json_result = None
        ...
        if not json_result:
            raise Exception(f"No JSON result found in output: {result.stdout}")
...
async def register_agent(registration: AgentRegistration):
    ...
    try:
        # Create agent object for governance identity tagging
        agent_data = {
            "id": registration.agent_id,
            "name": registration.name,
            "description": registration.description,
            "capabilities": registration.capabilities,
            "complianceLevel": "standard"  # Default compliance level
        }
        
        # Call governance identity module to tag agent
        identity_result = call_governance_identity("tagAgent", agent_data)
        ...
        return {
            "agent_id": registration.agent_id,
            "status": "registered",
            "governance_identity_id": identity_result.get("governanceIdentity", {}).get("agent_id", "unknown")
        }
