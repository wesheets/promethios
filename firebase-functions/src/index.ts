...
function generatePersonalizedName(data: WaitlistData): string {
...
function generateHTMLEmail(data: WaitlistData): string {
...
function generateTextEmail(data: WaitlistData): string {
  const name = generatePersonalizedName(data);
  const roleDisplay = roleDisplayMap[data.role] || data.role;
  const aiConcernDisplay = aiConcernDisplayMap[data.aiConcern] || data.aiConcern;
  const priorityDisplay = priorityDisplayMap[data.priority] || data.priority;
...
export const sendWaitlistEmail = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    try {
      const data = snap.data() as WaitlistData;
